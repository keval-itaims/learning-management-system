...
export class CourseService {
  ...
  public uploadCourseImage(
    ...
    return this.http.post<string[]>(
      `http://localhost:8080/course/save-course/${id}`,
      formData,
      {
        reportProgress: true,
        observe: 'events',
  ...
  addReview(userReview: UserReview): Observable<UserReview> {
    return this.http.post<UserReview>(
      `${this.url}/user-reviews/post-review`,
      userReview
    );
  }
  ...
  removeReview(id: number): Observable<any> {
    return this.http.delete<any>(`${this.url}/user-reviews/${id}`);
  }
